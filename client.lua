...
local PlayerData = TMGCore.Functions.GetPlayerData() -- Just for resource restart (same as event handler)
...
local function connecttoradio(channel)
    if channel > Config.MaxFrequency or channel <= 0 then TMGCore.Functions.Notify(Lang:t('restricted_channel_error'), 'error') return false end
    if Config.RestrictedChannels[channel] ~= nil then
        if not Config.RestrictedChannels[channel][PlayerData.job.name] or not PlayerData.job.onduty then
            TMGCore.Functions.Notify(Lang:t('restricted_channel_error'), 'error')
            return false
        end
    end
    ...
    if SplitStr(tostring(channel), ".")[2] ~= nil and SplitStr(tostring(channel), ".")[2] ~= "" then
        TMGCore.Functions.Notify(Lang:t('joined_to_radio', {channel = channel .. ' MHz'}), 'success')
    else
        TMGCore.Functions.Notify(Lang:t('joined_to_radio', {channel = channel .. '.00 MHz'}), 'success')
    end
    return true
end
...
local function leaveradio()
    ...
    TMGCore.Functions.Notify(Lang:t('you_leave') , 'error')
end
...
local function DoRadioCheck()
    hasRadio = TMGCore.Functions.HasItem(Config.RadioItem, 1)
end
...
RegisterNetEvent('tmg-radio:use', function()
    toggleRadio(not radioMenu)
end)

RegisterNetEvent('tmg-radio:onRadioDrop', function()
...
RegisterNUICallback('joinRadio', function(data, cb)
    local rchannel = TMGCore.Shared.Round(tonumber(data.channel), 2)
    if rchannel ~= nil then
        if rchannel <= Config.MaxFrequency and rchannel > 0 then
            if rchannel ~= RadioChannel then
                ...
                TMGCore.Functions.Notify(Lang:t('you_on_radio') , 'error')
            end
        else
            TMGCore.Functions.Notify(Lang:t('invalid_radio') , 'error')
        end
    else
        TMGCore.Functions.Notify(Lang:t('invalid_radio') , 'error')
    end
...
RegisterNUICallback('leaveRadio', function(_, cb)
    if RadioChannel == 0 then
        TMGCore.Functions.Notify(Lang:t('not_on_radio'), 'error')
    else
        leaveradio()
    end
    cb("ok")
...
RegisterNUICallback("volumeUp", function(_, cb)
	if RadioVolume <= 95 then
		...
		TMGCore.Functions.Notify(Lang:t("volume_radio", { value = RadioVolume}), "success")
		exports["pma-voice"]:setRadioVolume(RadioVolume)
	else
		TMGCore.Functions.Notify(Lang:t("decrease_radio_volume"), "error")
	end
    cb('ok')
end)
...
RegisterNUICallback("volumeDown", function(_, cb)
	if RadioVolume >= 10 then
		...
		TMGCore.Functions.Notify(Lang:t("increase_radio_volume"), "error")
	end
    cb('ok')
end)
...
RegisterNUICallback("increaseradiochannel", function(_, cb)
    ...
    local newChannel = TMGCore.Shared.Round(tonumber(RadioChannel + 1), 2)
